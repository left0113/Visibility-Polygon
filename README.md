...
目前透隨機散佈的點來進行覆蓋面積排序，排序方式是不在前一名的可見多邊形之下的最大面積的可見多邊形，經過排序就可以得出前六名的點(紅>橙>黃>綠>藍>紫)。這樣每次排序的結果都差不多，每一名大致都出現在相近的範圍內。不過依據此方法排序的出的位置有很大重複區域，通常較寬的道路上往往會有二到三個可見多邊形重疊，而最窄的道路則完全沒有覆蓋。因此之後的方法可以改變排序的方法，或許可以均等的覆蓋不同寬度的道路。
![ranking6](/viewerscenes/ranking6.jpg)
